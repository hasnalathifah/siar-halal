  ...
  return (
    <div className={containerClassName}>
      ...
      {readOnly && !(files?.length > 0) ? (
        <div className='divide-typo-outline text-typo-secondary border-typo-outline mt-1 divide-y rounded-[1.25rem]  border-2 py-3 pl-3 pr-4 text-sm'>
          Tidak ada file yang diupload
        </div>
      ) : files?.length >= maxFiles ? (
        <ul
          className={clsx([
            'divide-typo-outline border-typo-outline divide-y rounded-[1.25rem]  border-2',
            withLabel && 'mt-1',
          ])}
        >
          ...
          render={({ field: { value, ...rest } }) => (
            <>
              <div
                ...
                ref={dropzoneRef}
              >
                <input defaultValue={value} {...rest} {...getInputProps()} />
                <div
                  className={clsx(
                ...
                >
                  <div className='space-y-1 text-center'>
                    <svg
                      className='text-secondary-500 mx-auto h-12 w-12'
                      stroke='currentColor'
                      fill='none'
                      viewBox='0 0 48 48'
                      aria-hidden='true'
              ...
              {!readOnly && !!files?.length && (
                <ul className='divide-typo-outline border-typo-outline mt-1 divide-y rounded-[1.25rem]  border-2'>
                  {files.map((file, index) => (
                    <FilePreview
                      key={index}
                      readOnly={readOnly}
